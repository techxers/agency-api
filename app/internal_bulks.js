...
async function getBulkOutturnByIdandSeason(req, res) {
    const { outturnNo } = req.params;
    const { seasonID } = req.params;

    console.log('outturnNo ,  seasonID ' + id + ', ' + seasonID);

    try {
        const [rows] = await pool.query('SELECT * FROM grn_outturns WHERE outturnNo = ? and seasonID = ? ', [outturnNo,seasonID]);
        if (rows.length === 0) {

            res.status(404).json({ error: 'Bulk  record not found' });
        } else {
        ...
        res.status(500).json({ error: 'Failed to retrieve the Bulk record' + error.message });
    }
}
